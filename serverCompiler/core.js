...
var image = require("serverLib/rap.util.images.js");
...
exports = module.exports = {
   ...
   handleIncludeFile:function(file,outPath,watchFile){
     ...
     * 获取.{lang}.js的数据
     * */
    getLangData:function(abDataPath,matchLang,watchFile){
       var ret = {};
     ...
     * 获取语言配置
     * */
    getLangByFile:function(file,watchFile){
        ...
        if(/\.([a-z]+)\.html$/.test(fileName)){
            ...
            lang[matchLang] = exports.getLangData(abDataPath,matchLang,watchFile);
        }else{
            ...
            wake.findFile(filedir,"js").forEach(function (jsFile) {
                if(/\.([a-z]+)\.js$/.test(jsFile)){
                    ...
                    if(jsFileName.replace(".{0}.js".tpl(matchLang),".html")==fileName){
                        lang[matchLang] = exports.getLangData(jsFile,matchLang,watchFile);
                    }
                }
            })
    ...
    handleHTML:function(opt,watchFile){
        ...
        watchFile.include=function (incluedFile) {
            var lang = exports.getLangByFile(incluedFile,watchFile);
...
function handleOneFile(file,watchFile){
    ...
    //去掉注释
    var noteReg =/<!--.*?-->/gm
    var includeTags = fileData.replace(noteReg,"").match(includeReg);
...
function replaceIncludeHtml(currFileData,includeTag,includeFile,watchFile){
    ...
    //去掉注释
    var noteReg =/<!--.*?-->/gm
    var subIncludeTag = fileData.replace(noteReg,"").match(includeReg);
    if(subIncludeTag){
...
function watch(path){
    ...
    if(wake.isFile(absolutePath)){
        g_matchFile[absolutePath] = [];
        compile(absolutePath, g_matchFile[absolutePath]);
